...
function listJobs(db, filters) {
  ...
  const VALID_STATUSES = ['active', 'billable', 'voided', 'archived'];
  if (status) {
    ...
    // Default: show working jobs only (active + billable)
    sql += " AND j.status IN ('active', 'billable')";
  }

  if (group_id) {
    const descendantIds = getDescendantClientIds(db, group_id);
    const placeholders = descendantIds.map(() => '?').join(', ');
    sql += ` AND j.client_id IN (${placeholders})`;
